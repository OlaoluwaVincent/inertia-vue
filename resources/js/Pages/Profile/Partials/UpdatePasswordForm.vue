...
function handleCPassword() {
    if (form.password_confirmation !== form.password) {
        return notAMatch.value = true
    }
    return notAMatch.value = false
}
        ...
        <form @submit.prevent="updatePassword" class="tw-mt-6 tw-space-y-6">
            <div>
                ...
                <TextInput id="current_password" ref="currentPasswordInput" v-model="form.current_password"
                    type="password" class="tw-mt-1 tw-block tw-w-full" autocomplete="current-password" />
            ...
            <div>
                ...
                <TextInput id="password_confirmation" v-model="form.password_confirmation" type="password"
                    class="tw-mt-1 tw-block tw-w-full" autocomplete="new-password" @input="handleCPassword" />

                <InputError :message="form.errors.password_confirmation" class="mt-2" />
            </div>
            <p v-if="notAMatch" class="tw-text-red-700 tw-bold">Passwords do not match</p>

            <div class="flex items-center gap-4">
                <PrimaryButton :disabled="form.processing">Save</PrimaryButton>

                <Transition enter-active-class="tw-transition tw-ease-in-out" enter-from-class="tw-opacity-0"
                    leave-active-class="tw-transition tw-ease-in-out" leave-to-class="tw-opacity-0">
