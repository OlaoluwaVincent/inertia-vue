<template>
  <v-card v-for="(item, key) in data" :key="key"
    ...
    <v-img :src="item.image" cover class="img__height tw-relative">
      <div v-if="canDelete" class="tw-absolute tw-top-3 tw-left-3 tw-z-10 tw-w-full">
        <CourseCardDots :id="item.id" />
      </div>

      <v-btn v-if="!store.checkExisting(item.id) && !canDelete" @click="() => handleCartClick(item)"
        class="tw-absolute tw-top-3 tw-left-3 tw-z-10 tw-w-full" color="warning" icon="mdi-cart" variant="flat">
      </v-btn>
    </v-img>

    <v-card-item class="!tw-pb-0">
      <v-card-title class="tw-text-ellipsis tw-overflow-x-hidden !tw-text-sm md:!tw-text-base">
      ...
      <v-card-subtitle class="!tw-b-0">
        <span class="tw-me-1 tw-text-xs md:tw-text-base">By {{ item?.fullname || item.instructor.user.fullname }}</span>
      </v-card-subtitle>
    </v-card-item>

    <v-card-text>
      <div class="tw-flex tw-items-center tw-mx-0 tw-gap-4">
        <v-rating :model-value="item?.avg_rating" color="amber" density="compact" size="small" half-increments
          readonly></v-rating>
        <div class="tw-text-grey-400 tw-hidden md:tw-block" v-if="item?.avg_rating">
